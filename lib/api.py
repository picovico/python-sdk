...
class PicovicoAPIRequest:
	...
	def get(self, url=None, is_anonymous=False, auth_session=None):
		response = requests.get(urls.PICOVICO_API_ENDPOINT + url, headers=lambda auth_session: auth_session if auth_session else self.get_auth_headers())#if auth_session: return auth_session else self.get_auth_headers(is_anonymous))
