...
class PicovicoAPI2(PicovicoSessionMixin, pv_request.PicovicoRequest):
    def __init__(self, app_id, device_id=None, app_secret=None):
        super(PicovicoAPI, self).__init__(app_id, device_id=device_id, app_secret=app_secret)

    def authenticated_api(self, method='get', url, params=None, headers=None):
    ...
    def anonymous_api(method='get', url, params=None, headers=None):
        self.request_args = self.get_request_args(method, req_data=params)
        self.header = headers
        return self._respond(url)
